...
object Main {
    ...
    fun main() {
        ...
        // 重构建议： 卫语句取代嵌套
        if (targetFruit != null) {
            if (targetFruit.ripeness.colorRipeness < 0.5) {
                liuHuaQiang.goAway()
            } else {
                // 散弹式修改
                // 重构建议： 抽出常量、接口
                Log.e("REFACTOR", "多少钱一斤")
                val unitPrice = targetFruit.getUnitPrice() * targetFruit.getDiscount()
                ...
                if (notGo) {
                    ...
                    if (liuHuaQiang.getKillWilling(targetFruit.ripeness.value)) {
                        Log.e("REFACTOR", "你这瓜不熟")
